...
class Game:
    ...
    def AttackCell(self, x, y, boost=False):
        if self.token != '':
            headers = {'content-type': 'application/json'}
            r = requests.post(hostUrl + 'attack',
    ...
    def BuildBase(self, x, y):
        if self.token != '':
            headers = {'content-type': 'application/json'}
            r = requests.post(hostUrl + 'buildbase', data=json.dumps({'cellx': x, 'celly': y, 'token': self.token}),
                              headers=headers)
            if r.status_code == 200:
                data = r.json()
                if data['err_code'] == 0:
    ...
    def Boom(self, x, y, direction, boomType):
        if self.token != '':
            ...
            headers = {'content-type': 'application/json'}
            r = requests.post(hostUrl + 'boom', data=json.dumps(
                {'cellx': x, 'celly': y, 'token': self.token, 'direction': direction, 'boomType': boomType}),
                              headers=headers)
            if r.status_code == 200:
                data = r.json()
                if data['err_code'] == 0:
    ...
    def Refresh(self):
        ...
        if self.data == None:
            ...
            r = requests.post(hostUrl + 'getgameinfo', data=json.dumps({"protocol": 1, "timeAfter": self.lastUpdate}),
                              headers=headers)
            if r.status_code == 200:
                d = r.json()
